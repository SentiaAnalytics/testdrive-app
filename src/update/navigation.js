...
const handler:ActionHandler =  {
  HISTORY_PUSH: (state:Model, location:Dict) =>
    cmd({...state, location}, historyPush(state.user ? location : '/login')),
  HISTORY_POP: (state:Model,location:Dict) =>
    state.user ? ({...state, location}) : cmd({...state, location}, historyReplace('/login')),

  HISTORY_REPLACE: (state:Model, location:Dict) => cmd({...state, location}, historyReplace(state.user ? location : '/login')),
}
